import { prisma } from "@/lib/db";
import { runBizDevAgent } from "./actions";
import { TopNav } from "@/app/_components/TopNav";
import { queueCodexTask } from "@/app/actions";
import { AgentReportSurface } from "@/app/_components/AgentReportSurface";
import { AGENT_PROFILE_BY_ID } from "@/lib/agent-profiles";
import { getAgentMemory } from "@/lib/agent-memory";

export const dynamic = "force-dynamic";

export default async function BizDevAgentPage() {
  const [runs, memory] = await Promise.all([
    prisma.agentRun.findMany({
      where: { agentType: "bizdev" },
      orderBy: { startedAt: "desc" },
      take: 12,
    }),
    getAgentMemory("bizdev"),
  ]);
  const profile = AGENT_PROFILE_BY_ID.get("bizdev")!;

  return (
    <main className="min-h-screen">
      <TopNav
        title="BizDev Agent"
        subtitle="Research target corporations for snow removal + landscaping, then draft outreach."
      />

      <section className="mx-auto max-w-6xl px-6 py-6">
        <AgentReportSurface
          profile={profile}
          runs={runs}
          memory={memory}
          queueAction={queueCodexTask}
          runAction={runBizDevAgent}
          runFieldName="notes"
          runFieldPlaceholder="Optional: target regions, industries, brand proof points, constraints, or specific companies to avoid/revisit."
        />
      </section>
    </main>
  );
}
